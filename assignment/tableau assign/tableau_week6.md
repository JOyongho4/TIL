...
![설](./img/11120145.png)
```
지역별, 하위범주별, 월별 평균 배송 기간에 대한 대시보드입니다.
저는 필터링 기능을 통해 배송기간을 다양한 측면에서 한눈에 보는데 집중했습니다.

계산된 필드를 통해 배송 기간에 대한 필드를 만들었고 (배송일자-주문일자) 바둑판식으로 배치했습니다.
각 차트들은 필터 동작을 추가했습니다.
맵 차트의 특정 국가를 클릭하면 그 국가에 대한 하위범주별, 월별 평균 배송기간을 보여줍니다.
또한 월별 그래프와 하위 범주 그래프도 클릭 시 상대 그래프도 필터링되게 했습니다.
```
