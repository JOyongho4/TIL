...
```
라인차트는 데이터의 시간별 추세를 확인하거나 해당 데이터를 통해 미래값을 예측하려는 경우에 사용됨
월만을 열로 했을 때 연속형으로 설정하면 선이 이어짐
이중축 기능이용하여 두 칼럼간 비교 가능 그러나 양쪽의 세로축이 서로 다른 값을 나타내기 때문에 왜곡 가능성있음 -> 세로축 중 하나를 축 동기화
```
![설](./img/202409222110.png)
## 17강: 맵작성

<!-- 맵차트 관해 배우게 된 점을 적어주세요 -->
```
태블로는 지리적 분석에 유용한 맵을 작성할 수 있음. 경도와 위도 좌표가 있는 위치데이터를 이용.
테이블에 지리적 역할이 부여된 데이터 존재 (지구본 아이콘)
매출 등과 같은 데이터를 마크의 색상으로 끌어와 색상으로 나타낼 수 있음
이중축으로 두 데이터를 나타낼 수 있음
...
```
열,행,마크를 이용해 크로스탭 생성가능 (년,범주,매출합계)
분석-요약-총계를 이용해 총계 생성 가능
합계(매출)에서 비율로 표시 -> 테이블 옆으로, 테이블 아래로, 패널 아래로 등 선택
```
![설](./img/202409222136.png)
## 19강: 트리맵과 하이라이트테이블

```
트리맵: 비교적 간단한 시각화 방법으로써 직관적으로 데이터의 크기를 비교할 수 있는 장점이 있음. 데이터를 중첩된 사각형으로 표시하며 색상과 크기를 측정값으로 표기

레이블이 너무 많으면 트리맵으로 표현하는 의미가 없음 -> 필터 이용, 데이터 수 조절 필요

하이라이트 테이블: 색상이 입혀진 특수 테이블. 범주형 데이터를 색상으로 비교할 수 있기에 다양한 데이터를 나타내야 할때 용이

수익과 매출의 스케일이 다른데 스케일 반영이 되지 않아 매출은 진하게, 수익은 옅게 표시됨 -> 마크-색상 측정값-별도의 범례 사용
```
![설](./img/202409222149.png)
> *🧞‍♀️하이라이트 테이블 등에서 두개 이상의 측정값을 사용하는 경우, 함께 색을 표현하게 되면 단위가 달라 정확한 값을 표현할 수 없습니다. 이때 클릭해야 하는 항목은?*

```
마크-색상 측정값 우클릭-별도의 범례 사용
```
...
```
1. 트리맵 생성 -> 매출을 마크의 색상, 크기로 끌어옴

2. 'South Korea'인 데이터 만을 필터링 한다 -> 국가/지역 테이블을 필터로 끌어서 'South Korea'만 체크

3. 하위범주의 이름이 표시되게 한다 -> 하위범주를 마크의 레이블로 끌어옴

4. 하위범주별 매출의 비율을 표시한다 -> 매출을 마크의 레이블로 끌어옴, 퀵 테이블 계산을 구성비율로 변경
```
![설](./img/202409222204.png)
## 문제2
