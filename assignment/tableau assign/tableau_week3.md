...
```
위의 문제는 컨텍스트 필터를 사용하지 않아서 생긴 문제이다. 예를 들어, 배송날짜가 2021년 혹은 2022년인 것들 중 매출 상위 10개를 구해야 하는 것과, 매출 상위 10개인 것들 중 21년, 22년인 것들을 필터링 하는것은 차이가 있다. 이 문제를 해결하기 위해선 컨텍스트 필터를 이용해야 한다. 이를 이용하면 하나의 필터를 나머지 필터에 종속시켜, 21, 22년도인 값들 중 매출 상위 10개를 뽑는다. 필터에 년(배송날짜)를 우클릭하여 컨텍스트에 추가를 클릭하면 된다.
```
## 문제 2.
...
js
```
제품 이름의 필드를 우클릭, 만들기에 그룹에 들어간다. 그곳에서 <<찾기를 클릭, 그룹으로 묶을 제품들에 동일하게 들어가는 이름을 적어서 모두 찾기한다. (ex 삼성, Canon...) 선택된 제품들을 그룹들을 그룹을 눌러 그룹화하고 그룹이름을 문제에 지시된데로 변경한다. 마지막으로 필드명을 'Product Name Group'으로 변경하고 적용,확인한다
```
![설](./img/202409300120.png)
![설](./img/202409300124.png)
...
```
우선 Product Name Group의 그룹편집에서 '기타' 포함을 해제한다. 또한 행,열, 필터 그리고 마크까지 그림과 같이 설정을 한 후, 열은 측정값을 통해 카운트로 변환한다. 이 문제에서 중점은 필터라고 생각한다. 필터에서 우클릭, 필터 편집에 들어가 그룹화한 5 그룹만을 선택해야한다. 필터 편집에서 사용자 지정을 통해 그룹을 필터링하거나, 목록에서 선택할 수 있다. 이렇게 5개 그룹만을 설정하면 문제와 같은 그래프가 완성된다.
```
![설](./img/09300141.png)
![설](./img/09300136.png)