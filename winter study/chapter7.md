...
<MNIST로 이해>
    ...
    ㅇ 픽셀을 1차원으로 전환
    ㅇ 각각의 출력층 뉴런들이 각각의 입력층 픽셀값을 나름의 가중치 부여 후 계산
```
```
<활성화 함수>
...
< 다중 분류>
    ...
    ㅇ 이진 분류의 경우 1(양성)은 확률 p, 0(음성)은 곱해도 0이기에 1로 바꾸고 확률을 1-p로 함
    ㅇ 다중 분류는 타깃에 해당하는 확률만 남겨 놓기 위해 나머지 확률에는 모두 0을 곱함 (원핫인코딩)
```
![이미지](./img/02151024.png)
```
<텐서플로>
...
<은닉층의 활성화 함수>
    ...
    ㅇ 은닉층에서 단순 선형계산만 진행하면 수행역할이 없는셈
    ㅇ 선형을 비선형으로 적절히 바꿔야함
    ...
    여기서 b를 비선형으로 비틀지 않으면 사실상 아무 역할도 하지 못함
```
```
<시그모이드 함수>
...
**손실, 정확도**
fit()메서드를 실행하면 거기에 관한 손실과 정확도 값이 저장되어 있음
이 값들을 이용해서 그래프를 그릴 수 있음

```
<과적합>

    ㅇ 딥러닝에서 에포크를 늘리면 당연히 정확도가 늘고 손실이 준다
    ㅇ 그러나 당연히 머신러닝처럼 과적합의 문제가 있다
    ㅇ 검증 세트의 점수를 고려하여 에포크 수를 정해야 한다
```
**드롭아웃**
```
<드롭아웃>

    ㅇ 훈련 과정에서 일부의 뉴런을 랜덤하게 끔
    ㅇ 과적합 방지의 목적
    ㅇ 일부 뉴런이 랜덤하게 꺼지면 특정 뉴련에 과대하게 의존하는 것을 줄일 수 있음
    ㅇ 더 안정적임
    ㅇ 평가나 예측을 수행할 때는 드롭아웃을 적용하면 안됨 -> 텐서플로에서 자동으로 해줌

```
![이미지](./img/02151056.png)
```
<콜백>

    ㅇ 훈련 과정 중간에 어떤 작업을 수행할 수 있게하는 객체
    ㅇ 과적합 되기 전의 에포크를 수동으로 찾아 모델을 다시 훈련하는 번거로움을 줄임

<조기종료>

    ㅇ 과대적합이 시작되기 전에 훈련을 미리 중지하는 것
```